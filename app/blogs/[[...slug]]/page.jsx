...
async function getData(page = 1, catSlug = null) {
  ...
  try {
    if (catSlug) {
      const catRes = await fetch(`${baseUrl}/categories?slug=${catSlug}`, {
    ...
    const [postsRes, catsRes] = await Promise.all([
      ...
      fetch(`${baseUrl}/categories?per_page=30&_fields=id,name,slug`, {
        next: { revalidate: 3600 },
      }),
    ]);
            ...
            posts.map((post) => (
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <h2
                    className="text-xl font-extrabold text-[#12066a] dark:text-white mb-6 line-clamp-2 min-h-[3.5rem] leading-snug"
                    dangerouslySetInnerHTML={{ __html: post.title.rendered }}
                  />

                  <Link
                    href={`/${post.slug}/`}
                    className="mt-auto text-[#997819] font-black text-[11px] uppercase tracking-[0.2em] flex items-center gap-2 group/btn transition-all"
                  >
                    Read Full Story
